...
/// NOTE: `typename` is needed here because otherwise the compiler would think that
/// `linked_list<T>::iterator` is a static field of the class `linked_list<T>`, instead of
/// a type. Therefore, we need `typename` to tell the compiler `linked_list<T>::iterator`
/// is the name of a type.

/// More info: https://stackoverflow.com/questions/11275444/c-template-typename-iterator
template <typename T>
...
void linked_list<T>::sort(node *&head)
{
    ...
    /// NOTE: `exchange(obj, new_value)` replaces the value of obj with new_value
