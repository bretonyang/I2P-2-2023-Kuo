...
public:
    ...
    bool valid(State s)
    {
        ...
        // cannibals must not outnumber (i.e., more than) missionaries
    ...
    /**
     * Finds all solutions within `steps` steps.
     * Only collect direct paths to solution (i.e., no cyclic path inside the solution)
     * Or in other words, no duplicate states!
     */
        ...
        while (steps > 0) {
            ...
            for (auto p : _paths) {
                ...
                for (auto s : nextStates) {
                    ...
                    // not explored before (cend is a constant iterator)
                    else if (_explored.find(s) == cend(_explored)) {
                        auto np = p;
                        np.push_back(s);
                        newPaths.insert(np);
                    }
            ...
            // remove all old paths (paths in _paths before)
            // Why not just: _paths.clear()?
            for (auto p : oldPaths) {
                _paths.erase(p);
            }
            for (auto p : newPaths) {
